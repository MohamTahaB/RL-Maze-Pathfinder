...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, size=10, blacklist=list() ):
    ...
    def reset(self, first_reset=False, return_info=False, options=None):
        ...
        while self._agent_location.tolist()  in self.blacklist :
            self._agent_location = self.np_random.integers(0, self.size, size=2)
        ...
        if first_reset:
            self._target_location = self._agent_location
            while np.array_equal(self._target_location, self._agent_location) or self._target_location.tolist() in self.blacklist:
                self._target_location = self.np_random.integers(0, self.size, size=2)
    ...
    def step(self, action):
        ...
        #     reward = -5
        if self._agent_location.tolist() in self.blacklist :
            reward = -7
        else :
            if done:
                reward = 10
    ...
    def render(self, agent_qtable = None, mode="human"):
        ...
        # draw black circles in the places where there are walls
        for ar in self.blacklist :
            pygame.draw.rect(
                canvas,
                (96, 96, 96),
                pygame.Rect(
                    pix_square_size * np.array(ar),
                    (pix_square_size, pix_square_size),
                ),
            )
